...
const Map = () => {
  ...
  const token = (mapboxgl.accessToken =
    "pk.eyJ1IjoidGllbmRhdGdsIiwiYSI6ImNscDB5eTcyODA5ZGwyaXBqNDFrZjg4d2wifQ.v201xmqTHSI9wQ0JlM3RdQ");
  ...
  const handleGetDirection = async () => {
    ...
    try {
      ...
      setLoading(false);
      setShowCalculateDistance(false);
  ...
  const handleOptimizing = async () => {
    const matrixToken =
      "pk.eyJ1IjoidGllbmRhdGdsIiwiYSI6ImNscDB5cjMxODBkbHYycnFveG9oMW02czQifQ.1Gvp-oFUgvHOKCU1VmkSrw";
    try {
      ...
      class Graph {
        ...
        generateRandomPath() {
          ...
          for (let i = shuffledVertices.length - 1; i > 0; i--) {
            ...
            [shuffledVertices[i], shuffledVertices[j]] = [
              shuffledVertices[j],
              shuffledVertices[i],
            ];
          }
      ...
      if (path) {
        ...
        console.log(
          `Lộ trình ngắn nhất là ${path} with total distance ${totalDistance} km`,
        );
      } else {
      ...
      const sortedData = locationList.slice().sort((a, b) => {
        const keyA = newReorderArray.findIndex(
          (coords) => coords[0] === a.longitude && coords[1] === a.latitude,
        );
        const keyB = newReorderArray.findIndex(
          (coords) => coords[0] === b.longitude && coords[1] === b.latitude,
        );
        return keyA - keyB;
      });
    ...
    <div className="relative h-[94vh]">
      <div ref={mapContainer} className="h-[94vh] w-full">
        {routeInfo && (
          <>
            {showDistance && (
              <div className=" absolute left-0 top-0 z-50 ml-2 mt-2 rounded-md bg-white px-4 py-2 shadow-md">
                {locationList.length > 0 && (
                  <>
                    <p className="text-lg font-medium text-blue-500">
                      {formatDateTripList(locationList[0].day)} route :
                    </p>
                    ...
                    {showNewDistance && (
                      <div className="flex w-full items-center justify-between">
                        <button
                          ...
                          className=" w-full rounded-md bg-green-600 px-5 text-lg font-medium text-white hover:border-green-700 hover:bg-green-700"
                        >
                          Keep
                        </button>
                      </div>
            ...
            {showNewDistance && (
              <div className=" absolute right-2 top-0 z-50 ml-2 mt-2 rounded-md bg-white px-4 py-2 shadow-md">
                {locationList.length > 0 && (
                  <>
                    <p className="text-lg font-medium text-red-500">
                      New {formatDateTripList(locationList[0].day)} route :
                    </p>
                    <p className="text-lg font-medium text-slate-900">
                      Locations:{" "}
                      <span className=" text-blue-500">
                    ...
                    <p className="text-lg font-medium text-slate-900">
                      ...
                      <span className=" text-xl font-semibold text-red-500">
                        {(newDistance / 1000).toFixed(1)}
                      </span>{" "}
                      km{" "}
                    </p>
                    <div className="flex w-full items-center justify-between">
                      <button
                        onClick={handleKeepNewLocation}
                        className=" w-full rounded-md bg-green-600 px-5 text-lg font-medium text-white hover:border-green-700 hover:bg-green-700"
                      >
                        Acept
                      </button>
